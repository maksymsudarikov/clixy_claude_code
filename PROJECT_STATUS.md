...
**1. Токены для shoot pages** (НОВОЕ! 2025-12-28)
- Каждая съемка имеет уникальный `accessToken` (32 символа)
- Доступ к `/shoot/:id` только с валидным токеном: `?token=abc123...`
- Без токена → "Access Denied" 🔒
- Кнопка "Copy Link" в админке копирует ссылку с токеном
...
### ✅ Завершено (2025-12-28):
- [x] Token-based security для shoot pages
- [x] Landing page с Contact Hub
